...
| REQ-ID | Description | Priority | Phase | Status | Notes |
|--------|-------------|----------|-------|--------|-------|
| REQ-PROD-01 | Product catalog (dresses/shoes/bags/accessories/cosmetics) | P0 | 1 | In progress | 1.1 (`0003_catalog.sql` applied & types regen): `products` table + RLS (public read `active` / owner CRUD) + indexes live. List/detail UI = 1.6 |
| REQ-PROD-02 | Filters/sort (category, price, sort) | P1 | 1 | In progress | 1.1: filter/sort columns indexed — `(category_id,status)`, `(price)`, `(status)`, `(created_at desc)`, `(avg_rating desc)`. `useFilters`/FilterBar + query = 1.4/1.6 |
| REQ-PROD-03 | Product detail page | P0 | 1 | Not started | UI = 1.6 (`getProductById` + ProductGallery/VariantSelector) |
| REQ-PROD-04 | Variants (size/color/stock) | P1 | 1 | In progress | 1.1: `product_variants` table + unique `(product_id,size,color)` + RLS (public read where parent active / owner via join) live. Editor UI = 1.7 |
| REQ-PROD-05 | Seller CRUD (own products) | P0 | 1 | In progress | 1.1: owner-only RLS (`seller_id = auth.uid()`) live (deny-by-default). Write APIs + ProductManager = 1.7 |
| REQ-PROD-06 | Product images (Supabase Storage) | P0 | 1 | In progress | 1.2 **verified live**: `/api/upload` (user-session client, RLS-enforced) + `useUpload` + `ImageUploader` (reusable) emit the `[{url,alt,sort}]` shape for `products.images`; guards confirmed (BAD_FILE/TOO_LARGE/401). Wired into ProductForm = 1.7 |
...
| REQ-DASH-05 | Category management | P1 | 1 | In progress | 1.1: `categories` table + RLS (public read / service-role writes) + indexes live; **5 product categories seeded** (Dresses/Shoes/Bags/Accessories/Cosmetics, AR+EN). Admin API/UI = 1.5 |
